...
$identityuser = 'lcdautomate@cyberark.cloud.4077'
$usersecret = 'OT2T7Rbaf9YRp1Qi'
$domain = 'lab.local'

# Global Safe Name
$safeName = "broberts_lcd_test"
...
function autoAuth {
    ...
    try {
        $response = Invoke-RestMethod -Uri "https://aap4658.id.cyberark.cloud/oauth2/platformtoken" -Method POST -Headers $headers -Body $body -ErrorAction Stop
    } catch {
...
function getExistingAccount {
    ...
    $url = "https://west-isp.privilegecloud.cyberark.cloud/PasswordVault/API/Accounts?Safe=$safeName"
    $headers = @{
...
function userOnboard {
    ...
    $url = "https://west-isp.privilegecloud.cyberark.cloud/PasswordVault/API/Accounts/"
...
function passwordReset {
    ...
    if ($adminAccount) {
        ...
        $newPassword = "Silver$urfer123!"  # Temporary password for testing
...
function strongAccount {
    ...
    try {
        $response = Invoke-RestMethod -Uri "https://west-isp.dpa.cyberark.cloud/api/secrets/public/v1" -Method POST -Headers $headers -Body ($payload | ConvertTo-Json -Depth 99)
    } catch {
...
function Invoke-CyberArkTargetSet {
    ...
    $url = "https://west-isp.dpa.cyberark.cloud/api/targetsets/bulk"
